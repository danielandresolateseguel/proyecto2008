...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Función helper para agregar eventos táctiles y de click
    function addTouchAndClickEvent(element, handler) {
        if (element) {
            // Agregar evento click para compatibilidad general
    ...
    addTouchAndClickEvent(cartIcon, function() {
    ...
    if (floatingCart) {
        addTouchAndClickEvent(floatingCart, function() {
            shoppingCart.classList.add('active');
            overlay.classList.add('active');
            // Ocultar carrito flotante cuando se abre el carrito principal
            floatingCart.classList.remove('show');
    ...
    addTouchAndClickEvent(closeCart, function() {
    ...
    addTouchAndClickEvent(overlay, function() {
        shoppingCart.classList.remove('active');
        overlay.classList.remove('active');
        // Mostrar carrito flotante si hay productos
        if (floatingCart && cart.length > 0) {
    ...
    addToCartButtons.forEach(button => {
        addTouchAndClickEvent(button, function(event) {
    ...
    addTouchAndClickEvent(clearCartBtn, clearCart);
    
    // Evento para el botón de enviar por WhatsApp
    addTouchAndClickEvent(checkoutBtn, function() {
        if (cart.length === 0) {
    ...
    addTouchAndClickEvent(clearSearchBtn, function() {
    ...
    productCards.forEach((card) => {
        ...
        addTouchAndClickEvent(card, function(e) {
    ...
    if (closeModal) {
        addTouchAndClickEvent(closeModal, closeProductModal);
    }

    // Cerrar modal al hacer clic fuera del contenido
    if (productModal) {
        addTouchAndClickEvent(productModal, function(e) {
    ...
    if (modalAddToCartBtn) {
        addTouchAndClickEvent(modalAddToCartBtn, function(event) {
    ...
    function initializeCarousel() {
        ...
        indicators.forEach((indicator, index) => {
            addTouchAndClickEvent(indicator, () => {
                goToSlide(index);
                resetCarouselInterval();
            });
        });
